...
| `groupName` (via `packageRules`) | `cloudflare workers-sdk` for anything sourced from `cloudflare/workers-sdk` | `wrangler`, `@cloudflare/vite-plugin` and their siblings ship from one repo on one cadence, and the plugin asserts at load time that the installed `wrangler` satisfies its peer range — throwing outright when it does not, which a package manager deduplicating the tree can trigger. Splitting them across PRs leaves the build broken until both land. Renovate's monorepo data does not cover this repo, so `group:monorepos` misses it; matching by source URL rather than by name also keeps `@cloudflare/workers-types` out, since that ships from `cloudflare/workerd`. |
